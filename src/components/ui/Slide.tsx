...
const Slide = ({ pub, openModal, current, index, handleSlideClick }) => {
  const slideRef = React.useRef<HTMLLIElement>(null);
  ...
  return (
    ...
    >
        <div
            className="absolute top-0 left-0 w-full h-full bg-cover bg-center rounded-[1%] overflow-hidden transition-all duration-150 ease-out"
            style={{
      ...
      >
        <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <span className="text-xs text-slate-400" style={{ textShadow: '0px 0px 5px rgba(0,0,0,0.7)' }}>{pub.meta}</span>
        </div>
        <h3 className="text-lg font-bold text-white mb-3" style={{ textShadow: '0px 0px 8px rgba(0,0,0,0.8)' }}>{pub.title}</h3>
        <p className="text-sm text-slate-300 leading-relaxed mb-4" style={{ textShadow: '0px 0px 5px rgba(0,0,0,0.7)' }}>{pub.description}</p>
